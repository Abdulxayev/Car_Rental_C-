...
struct car {
	char plate_num[10];  // plate number with key events => char
	char brand[20];
	char model[20];
	float capacity;
	char colour[20];
...
void load(){
	...
	gotoxy(60,36);
	for(r=1;r<=timer;r++){
		for (q=0;q<=100000000; q++); //to displau the character slowly 
		printf("%c",117);
	}
	Sleep(100);
	system("cls");
...
void displayCar() {
	...
	for (int i = 0; i < countAvail(); i++){
		cout << "\t  | " << avail[i].plate_num << "\t\t" << avail[i].brand << "\t " <<avail[i].model <<"\t  " << avail[i].capacity << "\t   " << avail[i].colour << "\t " << avail[i].rate_per_hour << "\t\t    " << avail[i].rate_per_half  << "\t\t   " << avail[i].rate_per_day << "\t\t      " << avail[i].transmission << endl;
